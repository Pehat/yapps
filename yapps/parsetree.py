...
class Generator(object):
    ...
    def pretty_format_grammar(self):
        ...
        self.calculate()
        for r in self.goals:
            # ascii-graphics folder tab labeled with rule name
            yield  '    _____' + '_' * len(r)
            yield ('___/Rule ' + r + '\\' + '_' * 80)[:79]
            queue = deque([self.rules[r]])
            while queue:
                top = queue.popleft()
                yield 'Rule ' + repr(top) + ' of class ' + top.__class__.__name__
                top.first.sort()
                top.follow.sort()
                eps = []
                if top.accepts_epsilon:
                    eps = ['(null)']
                yield '     FIRST: ' + ', '.join(top.first + eps)
                yield '    FOLLOW: ' + ', '.join(top.follow)
    ...
    def generate_output(self):
        ...
        self.write("import logging\n")
        self.write("from yapps import runtime\n")
        self.write("\n")
        self.write("logging.basicConfig()\n")
        self.write("logger = logging.getLogger(__name__)\n")
        self.write("\n")
        ...
        if self.postparser is not None:
            ...
            self.write(INDENT, "else: logging.error('Args:  <rule> [<filename>]')\n")
            self.write("# End -- grammar generated by Yapps\n")
...
class NonTerminal(Node):
    ...
    def setup(self, gen):
        ...
        try:
            ...
            logger.error('Error: no rule <%s>', self.name)
            self.target = self
...
class Choice(Node):
    ...
    def output(self, gen, indent):
        ...
        for c in self.children:
            ...
            if removed:
                if not testset:
                    logger.error('Error in rule %s:', self.rule)
                else:
                    logger.warning('Warning in rule %s:', self.rule)
                logger.warning(' * %s', self)
                logger.warning(' * These tokens could be matched by more than one clause:')
                logger.warning(' * %s', ' '.join(removed))
...
class Option(Wrapper):
    ...
    def output(self, gen, indent):
        if self.child.accepts_epsilon:
            logger.warning('Warning in rule %s: contents may be empty.', self.rule)
...
class Plus(Wrapper):
    ...
    def output(self, gen, indent):
        if self.child.accepts_epsilon:
            logger.warning('Warning in rule %s:', self.rule)
            logger.warning(' * The repeated pattern could be empty.  The resulting parser may not work properly.')
...
class Star(Wrapper):
    ...
    def output(self, gen, indent):
        if self.child.accepts_epsilon:
            logger.warning('Warning in rule %s:', self.rule)
            logger.warning(' * The repeated pattern could be empty.  The resulting parser probably will not work properly.')
